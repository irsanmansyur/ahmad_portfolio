...
export default function EditPage({ sosmed }) {
  const [selectedURLImage, setSelectedURLImage] = useState(sosmed.url_icon);
  const { data, setData, post, processing, errors, reset, progress } = useForm({
    _method: 'put', name: sosmed.name, url: sosmed.url, value: sosmed.value, icon: sosmed.icons
  });
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route('admin.sosmed.update', sosmed.id));
  };
  return (
    <>
      <div>
