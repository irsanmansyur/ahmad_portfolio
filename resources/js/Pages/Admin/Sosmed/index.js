...
export default function SosmedPage({ sosmeds }) {
  return (
    <>
      <div id="payment-head" className="relative bg-sky-500 my-4 -ml-4 max-w-[90%] md:max-w-[75%] py-3 rounded-full rounded-bl-none px-[16px] shadow-md">
        <h1 className="title font-bold text-xl ml-3 text-white font-serif">List Sosial Media</h1>
      ...
      <div id="box-list-transaksi" className="p-3">
        <div className="head my-3">
          <Link className="rounded-lg bg-primary hover:opacity-80 text-white py-1 px-3 my-3" href={route("admin.sosmed.create")}>Add</Link>
        </div>
        <div className="inline-block overflow-x-auto w-full max-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
          <TableSosmeds sosmeds={sosmeds} />
        </div>
      </div>
    </>
  );
}
const TableSosmeds = ({ sosmeds }) => {
  return (<>
    {sosmeds.length < 1 ? <div className=" bg-red-500 rounded-lg p-3 text-white border border-white">Sosmed Empty</div> :
        ...
        <tbody className="bg-white">
          {sosmeds.map((sosmed, i) => (
            <tr key={i}>
              ...
              <td className="align-top px-2 md:px-6 py-2 md:py-4 whitespace-no-wrap border-b border-gray-200 text-xs">
                <img src={sosmed.url_icon} />

              </td>
              <td className="align-top px-2 md:px-6 py-2 md:py-4 whitespace-no-wrap border-b border-gray-200 text-xs">
                <div className="flex items-center justify-center">
                  <EditSosmed sosmed={sosmed} />
                  <DeleteSosmed sosmed={sosmed} />
                </div>
              </td>
            </tr>
          ))}
...
const EditSosmed = ({ sosmed }) => {
  const [loading, setLoading] = useState(false);
  return (<>
    {
      !loading ? <Link href={route("admin.sosmed.edit", sosmed.id)} onClick={e => setLoading(true)} className="inline-flex items-center justify-center p-[5px] bg-sky-500 text-white rounded-lg">
...
const DeleteSosmed = ({ sosmed }) => {
  const [openDelete, setOpenDelete] = useState(false);
  const { delete: destroy } = useForm();
  const onDelete = event => {
    event.preventDefault();
    destroy(route("admin.sosmed.destroy", sosmed.id), {
      preserveScroll: true,
...
SosmedPage.layout = page => <Authenticated props={page.props} children={page} />
